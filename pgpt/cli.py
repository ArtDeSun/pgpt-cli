...
from pgpt.maintenance import ingest, models, serve, status, sync
...
def cmd_chat_list(_args: argparse.Namespace) -> None:
    ...
    for slug, data in chats.list_chats():
        ...
        print(f"{marker} {slug:32} {data.get('project',''):12} {data.get('title','')}")
...
def cmd_chat(args: argparse.Namespace) -> None:
    ...
    print("Commands: /quit /new TITLE /web auto|on|off|research /context auto|on|off /deep auto|on|off\n")
    ...
    while True:
        ...
        data["messages"] = [*history, {"role": "user", "content": prompt}, {"role": "assistant", "content": result.answer}]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    def add_common_args(p: argparse.ArgumentParser) -> None:
        p.add_argument("--project")
        p.add_argument("-t", "--template", choices=["general", "explain-code", "debug", "implement", "architecture", "research"])
