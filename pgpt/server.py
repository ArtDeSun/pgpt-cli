...
from pgpt.maintenance import ingest_directory, resolve_knowledge_directory
...
def _extract_messages(payload: dict[str, Any]) -> tuple[str, list[dict[str, str]]]:
    ...
    for message in messages:
        ...
        if role in {"system", "user", "assistant"}:
            normalized.append(
                {"role": str(role), "content": _text_content(message.get("content"))}
            )
...
def _prepare_request(payload: dict[str, Any]) -> PreparedRequest:
    ...
    project = _optional_string(options, "project") if context is True else None
    if context is True and project is None:
        raise ValueError("Force project requires a selected project")
    return PreparedRequest(
        ...
        template=_choice(
            options,
            "template",
            {"general", "research", "explain-code", "debug", "implement", "architecture"},
        ),
        model=_optional_string(options, "model"),
        deep=_optional_bool(options, "deep"),
        history_mode=_choice(options, "history_mode", {"auto", "full", "off"}),
        answer_length=_choice(options, "answer_length", {"auto", "short", "standard", "long"}),
    )
...
def _timing_payload(timing: Any) -> dict[str, Any]:
    return {
        ...
        "phases": {
            ...
            for k, v in dict(getattr(timing, "phases", {})).items()
        },
        "metrics": dict(getattr(timing, "metrics", {})),
    }
...
def _project_details() -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for name, project in sorted(user_projects().items()):
        source = expand(str(project.get("source_dir", "")))
        rows.append(
            {
                "name": name,
                "source_dir": str(source),
                "exists": source.is_dir(),
                "collection": project.get("collection") or name,
            }
        )
    return rows
...
def _meta() -> dict[str, Any]:
    runtime = cfg_path("pgpt_home")
    legacy = runtime / "local_data" / "private_gpt"
    return {
        "name": "pgpt-cli",
        "projects": user_project_names(),
        "project_details": _project_details(),
        "project_registry": str(cfg_path("projects_file")),
        "privategpt_runtime": str(runtime),
        "legacy_privategpt_runtime": str(legacy) if legacy.exists() else None,
...
class PgptHandler(BaseHTTPRequestHandler):
    ...
    def _payload(self) -> dict[str, Any]:
        ...
        if length <= 0:
            return {}
        if length > int(CONFIG.get("server", {}).get("max_request_bytes", 2_000_000)):
    ...
    def _stream(self, payload: dict[str, Any]) -> None:
        ...
        def on_chunk(text: str) -> None:
            if not send(_stream_chunk(completion_id, created, content=text)):
    ...
    def _get_response(self, path: str) -> None:
        ...
        if download:
            self._bytes(
                ...
                headers={
                    "Content-Disposition": f'attachment; filename="{response_path.name}"'
                },
            )
            return
        self._json(
            HTTPStatus.OK,
            {
                **_response_metadata(response_path),
                "content": response_path.read_text(encoding="utf-8"),
            },
        )

    def _register_context(self, payload: dict[str, Any]) -> None:
        path = payload.get("path")
        name = payload.get("name")
        collection = payload.get("collection") or None
        if not isinstance(path, str) or not isinstance(name, str):
            raise ValueError("name and path are required strings")
        ...
        entry = save_user_project(name, str(root), collection=collection)
        self._json(
            HTTPStatus.OK,
            {
                "ok": True,
    ...
    def _ingest(self, payload: dict[str, Any]) -> None:
        ...
        if code != 0:
            raise RuntimeError(
                f"PrivateGPT ingestion failed with exit code {code}: "
                + " | ".join(lines[-5:])
            )
        self._json(
            HTTPStatus.OK,
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if path == "/api/context/register":
                self._register_context(payload)
                return
